...
class SimpleCNN(nn.Module):
    ...
    def _make_layer(self, in_channels, out_channels, num_blocks, stride):
        layers = []
        # Only the first block might change dimensions
        layers.append(ResidualBlock(in_channels, out_channels, stride))
        
        # The rest maintain dimensions
...
def main():
    ...
    CONFIG = {
        "model": "ResidualCNN",   # Change name when using a different model
    ...
    transform_train = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)), # CIFAR-100 specific normalization
    ])
    ###############
    ...
    optimizer = optim.SGD(model.parameters(), lr=CONFIG["learning_rate"], 
                         momentum=0.9, weight_decay=5e-4)   ### TODO -- define optimizer
